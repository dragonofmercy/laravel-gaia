<?php
return [

    'generic' => [
        'empty' => "<br />",
        'yes' => "Oui",
        'no' => "Non",
        'loading' => "Chargement...",
        'choose' => "Veuillez choisir une valeur",
        'success' => "Opération réussie !",
        'danger' => "Oups... une erreur est survenue !",
        'warning' => "Veuillez vérifier les informations",
        'info' => "Informations complémentaires",
    ],

    'datatable' => [
        'page_of_page' => "Page %s sur %s",
        'empty' => "Aucun élément pour le moment.",
        'empty_search' => "La recherche n'a pas retourné d'enregistrement.",
        'selection_no_elements' => "Veuillez sélectionner un élément dans la liste avant d'appuyer sur ce bouton !",
        'selection_single' => "Vous ne pouvez sélectionner qu'un seul élément !",
        'records' => "Affichage des résultats <b>:first</b> à <b>:last</b> (sur :total)",
        'search' => "Rechercher",
        'clear' => "Effacer",
        'close' => "Fermer",
        'filter' => "Filtrer",
        'reset' => "Vue par défaut",
    ],

    'component' => [

        'theme' => [
            'light' => "Lumos !",
            'dark' => "Nox !",
        ],

        'datepicker' => [
            'today' => "Aujourd'hui",
            'clear' => "Effacer",
            'now' => "Maintenant",
            'format' => [
                'date_format' => 'd.m.Y',
                'time_format' => 'H:i',
                'picker_date_format' => 'dd.MM.yyyy',
                'picker_time_format' => 'HH:mm'
            ]
        ],

        'copy' => [
            'notify' => "Copié !",
        ],

        'password' => [
            'title' => "Generateur de mot de passe",
            'new' => "Nouveau mot de passe",
            'choose' => "Choisir",
            'choose_copy' => "Choisir & copier",
        ],

        'checklist' => [
            'all' => "Tout sélectionner",
            'none' => "Tout désélectionner",
            'empty' => "Aucun élément pour le moment",
        ],

        'file' => [
            'browse' => "Parcourir",
            'selected' => "%d fichiers sélectionnés",
            'empty' => "Aucun fichier sélectionné",
        ],

        'image' => [
            'upload' => "Envoyer une nouvelle image",
            'trash' => "Supprimer l'image actuelle",
            'close' => "Fermer",
            'crop' => "Découper",
            'reset' => "Réinitialiser",
            'loading' => "Chargement..."
        ],

        'multifields' => [
            'add' => "Ajouter une ligne",
        ],

    ],

];